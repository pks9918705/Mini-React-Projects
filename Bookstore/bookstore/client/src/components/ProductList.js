...
const ProductList = () => {
  ...
  const [sortedProducts, setSortedProducts] = useState([...products]); // Keep a local state for sorted products
  const [minPrice, setMinPrice] = useState(0);
  const [maxPrice, setMaxPrice] = useState(3000);
  const [selectedType, setSelectedType] = useState('all'); // 'all' represents no type filter
